...
const editing = { id: null, name: '', phone: '', detail: '', isDefault: false }
let regionData = {}
let isDefault = false
...
async function init() {
  ...
  if (id) {
    editing.id = parseInt(id)
    const list = await api.address.getList()
    const addr = list.find(a => a.id === editing.id)
    if (addr) {
...
function renderForm() {
  ...
  provinceSelect.innerHTML = '<option value="">请选择省</option>' + Object.keys(regionData).map(p => `<option value="${p}">${p}</option>`).join('')
  updateDefaultSwitch()
}
...
async function saveAddress() {
  ...
  if (editing.id) {
    await api.address.update(editing.id, { name, phone, detail, isDefault })
  } else {
    await api.address.create({ name, phone, detail, isDefault })
  }
